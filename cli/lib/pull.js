/*
 Usage: scuba pull myStreamName -t LATEST/TRIM_HORIZON/AT_TIMESTAMP -d 1499408313497 -i shard-00000000001 -o folderName
 -t: shardIteratorType. Will default to LATEST
 -d: if shardIteratorType === 'AT_TIMESTAMP', timestamp to start reading at
 -i: specify a shardId – otherwise scuba will read from all shards at once
...
function getRecords(params, shardId, args, delay) {
    return kinesis.getRecords(params).promise().then((r) => {
        if (args.o) {
            var msBehind = r.Records.length ? '. ms behind is ' + r.MillisBehindLatest : '';
            console.log(`Found ${r.Records.length} records in ${shardId}${msBehind}`);
        }
        for (var a = 0; a < r.Records.length; a++) {
            if (args.o) {
...
module.exports = function (args) {
    ...
    kinesis.describeStream({StreamName: stream}).promise()
        .then((result) => {
            var iteratorType = determineShardIteratorType(args);
            var ts;
            ...
            if (args.i) {
                ...
                for (var i = 0; i < result.StreamDescription.Shards.length; i++) {
                    var shard = result.StreamDescription.Shards[i];
        ...
        .then((shardIterators) => {
            var getRecordsOps = [];
            for (var i = 0; i < shardIterators.length; i++) {
                var params = {
