...
module.exports = function(config, kinesis) {
  ...
  function getRecords(shard, attempts) {
    ...
    req.on('error', function (err) {
      ...
      if (err.code === 'ServiceUnavailable' || err.code === 'InternalFailure' || err.code === 'NetworkingError' || err.code === 'KMSThrottlingException') {
      ...
      if (resp.data.Records && resp.data.Records.length > 0) {
        ...
        config.processRecords.call(shard, resp.data.Records, function(err, checkpointShard) {
          ...
          if (checkpointShard) {
            ...
            var delay = getDelay(shard.lastGetRecords);
            setTimeout(getRecords.bind(this, shard, 0), delay);
          }
        });
      } else {
    ...
    function getDelay(startTime) {
      ...
      if (config.verbose) logger.info('Processing records took ' + diff + ' ms, delaying ' + delay + ' ms');
